...
@Component({
  ...
  template: `
    <app-card>
      @if (isValidating()) {
        <div class="py-8">
          <app-spinner text="Validando enlace..." />
        </div>
      } @else if (!tokenValid()) {
        <app-alert
          type="error"
        ...
        <div class="mt-4 text-center">
          <a routerLink="/recuperar-contrasena" class="text-atlas-teal dark:text-atlas-sage hover:underline">
...
export class ResetPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.token) {
      ...
      this.errorMessage.set('Enlace no proporcionado');
      return;
    }
    ...
    this.authService.validateToken(this.token).subscribe({
      next: (response) => {
        ...
        if (response.datos?.valido) {
          ...
          this.tokenValid.set(false);
          this.errorMessage.set('El enlace no es valido');
        }
      },
      error: (error) => {
        this.isValidating.set(false);
        this.tokenValid.set(false);
        this.errorMessage.set(error.error?.mensaje ?? 'Enlace invalido o expirado');
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    this.authService
      ...
      .subscribe({
        ...
        error: (error) => {
          this.isLoading.set(false);
          this.errorMessage.set(error.error?.mensaje ?? 'Error al restablecer la contrasena');
        },
      });
  }
}
