...
@Component({
  selector: 'app-icon',
  standalone: true,
  template: `<span [innerHTML]="getSafeSvg()" style="display:inline-flex;line-height:0;"></span>`,
...
export class IconComponent {
  ...
  getSafeSvg(): SafeHtml {
    const s = this.sizeMap[this.size()] ?? '20';
    const content = this.icons[this.name()] ?? '';
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${content}</svg>`;
